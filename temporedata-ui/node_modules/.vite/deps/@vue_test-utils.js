import {
  renderToString
} from "./chunk-GGYE5WNZ.js";
import {
  compile
} from "./chunk-YL5PYQ2A.js";
import {
  vue_runtime_esm_bundler_exports
} from "./chunk-2FWOMWYG.js";
import {
  BaseTransition,
  Fragment,
  Transition,
  ...
  transformVNodeArgs,
  unref
} from "./chunk-2YI3S6UW.js";
import "./chunk-NPIXSHU4.js";
...
var Pluggable = class {
  ...
  extend(instance) {
    const invokeSetup = ({ handler, options: options2 }) => {
...
function isClass(obj) {
  ...
  const isPrototypeCtorClass = prototype.constructor && prototype.constructor.toString && prototype.constructor.toString().substring(0, 5) === "class";
...
var mergeDeep = (target, source) => {
  ...
  Object.keys(source).concat(isClass(source) ? Object.getOwnPropertyNames((_a = Object.getPrototypeOf(source)) !== null && _a !== void 0 ? _a : {}) : Object.getOwnPropertyNames(source)).forEach((key) => {
...
function convertStubsToRecord(stubs2) {
  if (Array.isArray(stubs2)) {
    return stubs2.reduce((acc, current) => {
      acc[current] = true;
      return acc;
    }, {});
  }
...
var isDirectiveKey = (key) => key.match(/^v[A-Z].*/);
function getComponentsFromStubs(stubs2) {
  const normalizedStubs = convertStubsToRecord(stubs2);
...
function getDirectivesFromStubs(stubs2) {
  const normalizedStubs = convertStubsToRecord(stubs2);
  return Object.fromEntries(Object.entries(normalizedStubs).filter(([key, value]) => isDirectiveKey(key) && value !== false).map(([key, value]) => [key.substring(1), value]));
}
...
function getEventProperties(eventParams) {
  const { modifiers, options: options2 = {} } = eventParams;
  ...
  const keyCode = keyCodesByKeyName[keyModifiers[0]] || options2 && (options2.keyCode || options2.code);
  const eventProperties = Object.assign(Object.assign(Object.assign(Object.assign({}, systemModifiersMeta), options2), {
    bubbles: meta.bubbles,
    cancelable: meta.cancelable,
    // Any derived options should go here
    keyCode,
    code: options2.code || codesByKeyName[keyModifiers[0]] || keyCode
...
function createDOMEvent(eventString, options2) {
  ...
  if (options2) {
    Object.keys(options2).forEach((key) => {
      ...
      const canSetProperty = !(propertyDescriptor && propertyDescriptor.set === void 0);
      if (canSetProperty) {
...
var getComponentNameInSetup = (instance, type) => Object.keys((instance === null || instance === void 0 ? void 0 : instance.setupState) || {}).find((key) => {
  var _a;
  return ((_a = Object.getOwnPropertyDescriptor(instance.setupState, key)) === null || _a === void 0 ? void 0 : _a.value) === type;
});
var getComponentRegisteredName = (instance, type) => {
...
var getComponentName = (instance, type) => {
...
function find(root, selector) {
  ...
  if (typeof selector === "string") {
    matchingVNodes = matchingVNodes.filter((vnode) => {
      var _a;
...
function isElementVisible(element) {
  return element.nodeName !== "#comment" && isStyleVisible(element) && isElementVisibleByAttribute(element) && !isHiddenByClosedDetails(element) && (!element.parentElement || isElementVisible(element.parentElement));
}
...
var createDOMWrapper = (element, subTree) => factories[WrapperType.DOMWrapper](element, subTree);
var createVueWrapper = (app, vm, setProps) => factories[WrapperType.VueWrapper](app, vm, setProps);
...
var BaseWrapper = class {
  ...
  constructor(element) {
    this.isDisabled = () => {
      ...
      const hasDisabledAttribute = this.attributes().disabled !== void 0;
  ...
  findComponent(selector) {
    ...
    if (typeof selector === "object" && "ref" in selector) {
      let result2 = currentComponent.refs[selector.ref];
      if (Array.isArray(result2)) {
        result2 = result2.length ? result2[0] : void 0;
      }
      if (result2 && !(result2 instanceof HTMLElement)) {
        return createVueWrapper(null, result2);
      } else {
        return createWrapperError("VueWrapper");
      }
    }
  ...
  findAllComponents(selector) {
    ...
    return results.map((c) => c.proxy ? createVueWrapper(null, c.proxy) : createDOMWrapper(c.vnode.el, c.subTree));
  }
  html(options2) {
    ...
    return stringNodes.map((node) => beautify.html(node, {
  ...
  classes(className) {
    const classes = isElement(this.element) ? Array.from(this.element.classList) : [];
    if (className)
  ...
  trigger(eventString, options2) {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      if (this.element && !this.isDisabled()) {
        const event = createDOMEvent(eventString, options2);
...
var DOMWrapper = class _DOMWrapper extends BaseWrapper {
  ...
  getRootNodes() {
    var _a, _b;
    if (((_a = this.subTree) === null || _a === void 0 ? void 0 : _a.type) === Fragment && Array.isArray((_b = this.subTree) === null || _b === void 0 ? void 0 : _b.children)) {
  ...
  setSelected() {
    ...
    const parentWrapper = new _DOMWrapper(parentElement);
...
function getRootNodes(vnode) {
  if (vnode.shapeFlag & 1) {
    ...
    return children.flatMap((vnode2) => getRootNodes(vnode2)).filter(isNotNullOrUndefined);
  }
  return [];
}
var events = {};
var recordedUidsByRoot = /* @__PURE__ */ new Map();
...
var recordEvent = (vm, event, args) => {
  ...
  if (!recordedUidsByRoot.has(rootCid)) {
    recordedUidsByRoot.set(rootCid, /* @__PURE__ */ new Set());
  }
...
function createVMProxy(vm, setupState) {
  return new Proxy(vm, {
    get(vm2, key, receiver) {
      if (vm2.$.exposeProxy && key in vm2.$.exposeProxy) {
        return Reflect.get(vm2.$.exposeProxy, key, receiver);
      } else if (vm2.$.exposed && key in vm2.$.exposed) {
        return unref(Reflect.get(vm2.$.exposed, key, receiver));
        ...
        return Reflect.get(vm2, key);
      } else {
        return vm2.$.ctx[key];
      }
    },
    set(vm2, key, value, receiver) {
    ...
    defineProperty(vm2, key, attributes) {
      if (key in setupState) {
    ...
    getOwnPropertyDescriptor(vm2, property) {
...
var VueWrapper = class extends BaseWrapper {
...
function processSlot(source = "", Vue$1 = vue_runtime_esm_bundler_exports) {
...
var mountedRootComponents = /* @__PURE__ */ new WeakSet();
var registerRootComponent = (type) => {
  if (type && typeof type !== "string") {
    mountedRootComponents.add(type);
  }
};
var isRootComponent = (rootComponents, type, instance) => !!(!instance || // Don't stub mounted component on root level
(rootComponents.component === type || mountedRootComponents.has(type)) && !(instance === null || instance === void 0 ? void 0 : instance.parent) || // Don't stub component with compat wrapper
rootComponents.functional && rootComponents.functional === type);
var createVNodeTransformer = ({ rootComponents, transformers }) => {
  const transformationCache = /* @__PURE__ */ new WeakMap();
...
var clearAndUpper = (text) => text.replace(/-/, "").toUpperCase();
var kebabToPascalCase = (tag) => tag.replace(/(^\w|-\w)/g, clearAndUpper);
var DEFAULT_STUBS = {
  teleport: isTeleport,
...
var createDefaultStub = (kebabTag, predicate, type, stubs2) => {
...
var createStub = ({ name, type, renderStubDefaultSlot }) => {
  const anonName = "anonymous-stub";
  const tag = name ? `${hyphenate(name)}-stub` : anonName;
  const componentOptions = type ? unwrapLegacyVueExtendComponent(type) || {} : {};
...
var resolveComponentStubByName = (componentName, stubs2) => {
  for (const [stubKey, value] of Object.entries(stubs2)) {
    if (matchName(componentName, stubKey)) {
      return value;
    }
  }
};
function createStubComponentsTransformer({ rootComponents, stubs: stubs2 = {}, shallow = false, renderStubDefaultSlot = false }) {
  return function componentsTransformer(type, instance) {
    var _a, _b, _c;
    for (const tag in DEFAULT_STUBS) {
      const predicate = DEFAULT_STUBS[tag];
      const defaultStub = createDefaultStub(tag, predicate, type, stubs2);
      if (defaultStub)
        return defaultStub;
    }
    ...
    if (isComponent(stub)) {
      ...
      const specializedStubComponent = stubFn ? (...args) => stubFn(...args) : Object.assign({}, unwrappedStub);
...
function createStubDirectivesTransformer({ directives: directives2 = {} }) {
  if (Object.keys(directives2).length === 0) {
    return (type) => type;
  }
  return function directivesTransformer(type) {
    if (isObjectComponent(type) && type.directives) {
      const directivesToPatch = Object.keys(type.directives).filter((key) => key in directives2);
...
var deeplyCheckForRef = (r, visitedObjects) => {
  if (isRef(r))
    return true;
  if (!isObject(r))
    return false;
...
function getInstanceOptions(options2) {
  ...
  const resultOptions = Object.assign({}, options2);
...
function createInstance(inputComponent, options2) {
  ...
  let component;
  const instanceOptions = getInstanceOptions(options2 !== null && options2 !== void 0 ? options2 : {});
  ...
  if (isFunctionalComponent(originalComponent) || isLegacyFunctionalComponent(originalComponent)) {
    ...
    }, props: originalComponent.props || {}, setup: (props2, { attrs, slots: slots2 }) => () => h(originalComponent, Object.assign(Object.assign({}, props2), attrs), slots2) }, instanceOptions));
  ...
  const slots = (options2 === null || options2 === void 0 ? void 0 : options2.slots) && Object.entries(options2.slots).reduce((acc, [name, slot]) => {
  ...
  if (options2 === null || options2 === void 0 ? void 0 : options2.data) {
    const providedData = options2.data();
  ...
  Object.entries(Object.assign(Object.assign(Object.assign(Object.assign({}, options2 === null || options2 === void 0 ? void 0 : options2.attrs), options2 === null || options2 === void 0 ? void 0 : options2.propsData), options2 === null || options2 === void 0 ? void 0 : options2.props), { ref: MOUNT_COMPONENT_REF })).forEach(([k, v]) => {
    if (isDeepRef(v)) {
      refs[k] = v;
    } else {
      props[k] = v;
  ...
  const global2 = mergeGlobalProperties(options2 === null || options2 === void 0 ? void 0 : options2.global);
  if (isObjectComponent(component)) {
    component.components = Object.assign(Object.assign({}, component.components), global2.components);
  }
    ...
    const mixin = {
      beforeCreate() {
        if (isScriptSetup(this)) {
          for (const [k, v] of Object.entries(global2.mocks)) {
            try {
              this.$.setupState[k] = v;
            } catch (e) {
            }
          ...
          this.$.proxy = new Proxy(this.$.proxy, {
            get(target, key) {
              if (key in global2.mocks) {
                return global2.mocks[key];
              }
              return target[key];
            }
          });
  ...
  if (global2.config) {
    for (const [k, v] of Object.entries(global2.config)) {
      app.config[k] = isObject(app.config[k]) ? Object.assign(app.config[k], v) : v;
    }
  }
  if (global2.provide) {
    for (const key of Reflect.ownKeys(global2.provide)) {
      app.provide(key, global2.provide[key]);
    }
  }
  if (global2.plugins) {
    for (const plugin of global2.plugins) {
  ...
  if (global2.mixins) {
    for (const mixin of global2.mixins)
      app.mixin(mixin);
  }
  if (global2.components) {
    for (const key of Object.keys(global2.components)) {
  ...
  if (global2.directives) {
    for (const key of Object.keys(global2.directives))
  ...
  if (global2 === null || global2 === void 0 ? void 0 : global2.stubs) {
    for (const name of Object.keys(getComponentsFromStubs(global2.stubs))) {
...
function mount(inputComponent, options2) {
  const { app, props, componentRef } = createInstance(inputComponent, options2);
  ...
  if (options2 === null || options2 === void 0 ? void 0 : options2.attachTo) {
    ...
    if (typeof options2.attachTo === "string") {
      ...
      if (!to) {
        throw new Error(`Unable to find the element matching the selector ${options2.attachTo} given as the \`attachTo\` option`);
      }
    } else {
...
var shallowMount = (component, options2) => {
  return mount(component, Object.assign(Object.assign({}, options2), { shallow: true }));
};
function renderToString2(component, options2) {
  if (options2 === null || options2 === void 0 ? void 0 : options2.attachTo) {
    console.warn("attachTo option is not available for renderToString");
  }
  const { app } = createInstance(component, options2);
